...
int main() {
    ...
    std::cout << "sono state fatte " << numero_estrazioni << " estrazioni" << std::endl;
    ...
    while (true) {
        ...
        for (auto x : lista_studenti) {
            std::cout << std::setw(13) << x.name <<" -- punteggio:" << std::setw(2) << x.score << " -- interrogato a ";
            x.materie_interrogato();
            std::cout << interrogazioni.size()<<" materie:";
        ...
        while (true) {
            ...
            if (scelta == 's' || scelta == 'S') {
                ...
                lista_studenti[n_seriale].modifica_punteggio(-5);
            ...
            if (scelta == 'a' || scelta == 'A') {
                ...
                if (estrazioni > 0) {
                    ...
                    while (estratti.size() < estrazioni) {
                        if (std::find(estratti.begin(), estratti.end(), lista_stringhe[count]) == estratti.end()) {
                    ...
                    for (auto estratto : estratti) {
                        for (size_t n_seriale{ 0 }; n_seriale < nomi.size(); n_seriale++) {
                            if (estratto == nomi[n_seriale]) {
                                lista_studenti[n_seriale].modifica_punteggio(-3);
                                lista_studenti[n_seriale].vengo_interrogato(materia);
                            }
                        }
                    }
